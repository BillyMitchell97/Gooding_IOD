...
!-----NOTES-----------------------------------------
!-----INPUTS ARE:
!-----R1/2/3 = (OX1/2/3, OY1/2/3, OZ1/2/3)
!-----EACH R=VECTOR C->O, WHERE C=CENTRE OF FORCE, O = OBSERVING SITE
!-----LAMBDA1/2/3 = (EL1/2/3, EM1/2/3M EN1/2/3)
!-----EACH LAMBDA  = OBSERVED-DIRECTION AFFINE VECTOR
!-----T12 = T1-T2, T13 = T1-T3 WHERE EACH T = TIME OF OBSERVATION

!-----OUTPUTS ARE:
!-----RHO1/3
!-----EACH RHO IS THE VECTOR O->P WHERE
!-----O = OBSERVING SITE, P = POSITION IN ORBIT
...
!---------------------NEEDED IF SERIES AND OTHERWISE USEFUL WHEN Z=?
              ...
              IF (LM1 .OR. M.GT.0 .OR. X.LT.0D0 .OR. DABS(U).GT.SW) THEN
                      PRINT *, "IN IF: LM1.OR.M.GT.0.OR.X.LT.0.OR.ABS(U)
     1.GT.SW"
...
!---------------------DIRECT COMPUTATION (NOT SERIES)
                      ...
                      IF (QX.EQ.0D0.AND.LM1 .OR. QX.GT.0D0) THEN
                              PRINT *, "IN IF: QX.EQ.0.AND.LM1.OR.QX.GT
     1.0"
...
!-------------SINGLE-REV STARTER FROM T (AT X=0) & BILINEAR USUALLY
                      ...
                      PRINT *, "ENTERING FIRST XLAMB->TLAMB"
                      CALL TLAMB(M,Q,QSQFM1,0D0,0,T0,DT,D2T,D3T)
                      PRINT *, "EXITING FIRST XLAMB->TLAMB"

                      TDIFF = TIN - T0
                      IF (TDIFF.LE.0D0) THEN
                              PRINT *, "IN IF: TDIFF.LE.0"
...
!---------------------STARTER FOR TMIN
                      ...
                      PRINT *, "ENTERING 2ND XLAMB->TLAMB"
                      CALL TLAMB (M,Q,QSQFM1,XM,3,TMIN,DT,D2T,D3T)
                      PRINT *, "EXITING 2ND XLAMB->TLAMB"

                      IF (D2T.EQ.0D0) GO TO 2
                      PRINT *, "IN IF: D2T.EQ.0D0"
...
!-------------(NOW HAVE A STARTER, SO PRECEED BY HALLEY)
    ...
    5         CONTINUE
              ...
              PRINT *, "ENTERING 3RD XLAMB->TLAMB"
              CALL TLAMB (M, Q, QSQFM1, X, 2, T, DT, D2T, D3T)
              PRINT *, "EXITING 3RD XLAMB->TLAMB"
...
!-------------(SECOND MULTI-REV STARTER)
              ...
              PRINT *, "ENTERING 4TH XLAMB->TLAMB"
    3         CALL TLAMB (M, Q, QSQFM1, 0D0, 0, T0, DT, D2T, D3T)
              PRINT *, "EXITING 4TH XLAMB->TLAMB"
...
!-----------------------------(NO FINITE SOLUTION WITH X < XM)
     ...
     1           VT12, VR21, VT21, VR22, VT22)
              ...
              PRINT *, "ENTERING FIRST VALAMB->XLAMB"
              CALL XLAMB (M, Q, QSQFM1, T, N, X1, X2)
              PRINT *, "EXITING FIRST VALAMB->XLAMB"
...
!-------------PROCEED FOR SINGLE SOLUTION, OR A PAIR
              ...
              PRINT *, "ENTERING FIRST VALAMB->TLAMB"
              CALL TLAMB (M,Q,QSQFM1,X,-1,UNUSED,QZMINX,QZPLX,ZPLQX)
              PRINT *, "EXITING FIRST VALAMB->TLAMB"
...
!-------------EXTERNALLY GIVEN LINE OF SIGHT)
     ...
     1          R3KNSQ(9)
              ...
              PRINT *, "ENTERING FIRST CALCPS VECMUL"
              PRINT *, "FIRST VECTOR = ", X1, Y1, Z1
              PRINT *, "2ND VECTOR  = ", X3, Y3, Z3
              CALL VECMUL (X1, Y1, Z1, X3, Y3, Z3, X13, Y13, Z13)
              PRINT *, "OUTPUT VECTOR = ", X13, Y13, Z13
              PRINT *, "EXITING FIRST CALCPS VECMUL"
...
!-------------(FAILS ONLY IF EITHER R1 OR R2 IS ZERO
              ...
              PRINT *, "ENTERING FIRST CALCPS->VALAMB"
              CALL VALAMB (GM, R1, R3, TH, T13, NUM, VR1, VT1, VR3, VT3,
     1                     WR1, WT1, WR3, WT3)
              PRINT *, "EXITING FIRST CALCPS->VALAMB"

              IF (NUM.GT.0) THEN

                      PRINT *, "IN IF: NUM.GT.0"

                      IF (IND.EQ.1) THEN
                              PRINT *, "IN IF: IND.EQ.1"
                      ...
                      PRINT *, "ENTERING 2ND CALCPS VECMUL"
                      CALL VECMUL (X1, Y1, Z1, X3, Y3, Z3, XN, YN, ZN)
                      PRINT *, "EXITING 2ND CALCPS VECMUL"

                      PRINT *, "ENTERING 3RD CALCPS VECMUL"
                      CALL VECMUL (XN, YN, ZN, X1, Y1, Z1, XT, YT, ZT)
                      PRINT *, "EXITING 3RD CALCPS VECMUL"
                      ...
                      PRINT *, "ENTERING PV3ELS"
                      PRINT *, "AL, ALV, ALW: ", AL, ALV, ALW
                      CALL PV3ELS (GM, X1, Y1, Z1, V1X1, V1Y, V1Z, AL,
     1                             Q, EI, BOM, OM, TAU)
                      PRINT *, "EXITING PV3ELS"
                      PRINT *, "AL = ", AL

                      PRINT *, "ENTERING ELS3PV"
!                      CALL ELS3PV (GM, ALV, Q, EI, BOM, OM, TAU+T12, X2,
!     1                             Y2, Z2, W1, W2, W3)
                      CALL ELS3PV (GM, AL, Q, EI, BOM, OM, TAU+T12, X2,
     1                             Y2, Z2, W1, W2, W3)
                      PRINT *, "EXITING ELS3PV"
...
!-----ORBIT FROM THREE OBSERVED LINES OF SIGHT (ANGLES ONLY)
    ...
    1         PDINCM = PDINC
              IF (LOBDEP .AND. IKN.EQ.0 .AND. RO1.EQ.RO3) THEN
                      ...
                      PRINT *, "IN IF: LOBDEP.AND.IKN.EQ.0.AND.RO1.EQ
     1.RO3"

                      PRINT *, "ENTERING INITIAL CALCPS"
                      CALL CALCPS(NHREV,RO1,RO3,IND,NUM,CX,CY,CZ)
                      PRINT *, "EXITING INITIAL CALCPS"
                      ...
                      IF (NUM.GT.0) THEN
                              ...
                              IF (CR.LT.0D0) THEN
                                      ...
                                      PRINT *, "ENTERING 2ND CALCPS"
                                      CALL CALCPS(NHREV,RO1+DR,RO3+DR,
     1                                            IND,NUM,CX,CY,CZ)
                                      PRINT *, "EXITING 2ND CALCPS"
    ...
    2         NMODS = 0
              ...
              PRINT *, "ENTERING 3RD CALCPS"
    3         CALL CALCPS(NHREV,RO1,RO3,IND,NUM,CX,CY,CZ)
              PRINT *, "EXITING 3RD CALCPS"

              R10 = R1
              R30 = R3
              IF (NUM.EQ.0) THEN
                      PRINT *, "IN IF: NUM.EQ.0 (2)"

                      IF (ITNUM.GT.1 .AND. NFLMOD.LT.3) THEN
                              PRINT *, "IN IF: ITNUM.GT.1.AND.NFLMOD
     1.LT.3"
     ...
     1                                    RO3 = 0D0
              ...
              PRINT *, "ENTERING FIRST VECMUL"
              CALL VECMUL(EL2,EM2,EN2,CX,CY,CZ,ENX,ENY,ENZ)
              PRINT *, "EXITING FIRST VECMUL"

              PRINT *, "ENTERING 2ND VECMUL"
              CALL VECMUL(ENX,ENY,ENZ,EL2,EM2,EN2,PX,PY,PZ)
              PRINT *, "EXITING 2ND VECMUL"

              PR = RNORM(PX, PY, PZ)

              PRINT *, "ENTERING 3RD VECMUL"
              CALL VECMUL(EL2,EM2,EN2,PX,PY,PZ,ENX,ENY,ENZ)
              PRINT *, "EXITING 3RD VECMUL"

              ENR = RNORM(ENX, ENY, ENZ)

              IF (ENR.EQ.0D0) THEN
                      PRINT *, "IN IF: ENR.EQ.0"
    ...
    4         CONTINUE
     ...
     1             NMODS.LT.2) THEN
                      ...
                      NMODS = NMODS + 1
                      PRINT *, "IN IF: ITNUM.GT.1.AND.FC.GT.2D0*FCOLD
     1.AND.NMODS.LT.2"
     ...
     1               0.5D0*(GDD1*ROFAC + GDD3/ROFAC)
              ...
              DO 5 I=1,5
              IF (NLDF(I).EQ.0) THEN
                      PRINT *, "IN IF: NLDF(I).EQ.0"

                      NPDF = NPDF + 1

                      IF (NPDF.LE.3) THEN
                              PRINT *, "IN IF: NPDF.LE.3"
    ...
    6         CONTINUE
              ...
              IF (DABS(AXRTIO).LT.CRITGM) THEN
                      PRINT *, "IN IF: ABS(AXRTIO).LT.CRITGM"
                      PRINT *, "AXRTIO = ", AXRTIO
                      PRINT *, "CRITGM = ", CRITGM
    ...
    9         NFAIL = 0
              ...
              PRINT *, "ENTERING LAST CALCPS"
              CALL CALCPS(NHREV,RO1,RO3,IND,NUM,CX,CY,CZ)
              PRINT *, "EXITING LAST CALCPS"
     ...
     1                       OM,TAU)
              ...
              PRINT *, "ENTERING PV2ELS"
              CALL PV2ELS(GM,R,U,VR,VT,AL,Q,OM,TAU)
              PRINT *, "EXITING PV2ELS"
      ...
      SUBROUTINE PV2ELS(GM,R,U,VR,VT,AL,Q,OM,TAU)
              ...
              PRINT *, "CALCULATING AL"
              PRINT *, "GM, R, VSQ = ", GM, R, VSQ 
              AL = 2D0*GM/R - VSQ
              ALP = DABS(AL)
              RTAL = DSQRT(ALP)
              D = R*VR
     ...
     1                      0.5D0),OM)
      ...
      SUBROUTINE ELS3PV(GM,AL,Q,EI,BOM,OM,TAU,X,Y,Z,XDOT,YDOT,ZDOT)
              ...
              PRINT *, "ENTERING ELS2PV"
              CALL ELS2PV(GM,AL,Q,OM,TAU,R,U,VR,VT)
              PRINT *, "EXITING ELS2PV"
